...
BEGIN
    ...
    FROM (
        SELECT 
            ...
            COALESCE(COUNT(DISTINCT CASE WHEN event_type = 'impression' THEN COALESCE(metadata->>'visitor_id', id::text) END), 0) as impressions,
            COALESCE(COUNT(DISTINCT CASE WHEN event_type = 'widget_open' THEN COALESCE(metadata->>'visitor_id', id::text) END), 0) as opens,
            COALESCE(COUNT(DISTINCT CASE WHEN event_type = 'game_complete' THEN COALESCE(metadata->>'visitor_id', id::text) END), 0) as conversions
