-- Update check_widget_status to check for available stock
...
BEGIN
    ...
    -- Check if ANY coupon has stock (quantity > used_count)
    SELECT EXISTS (
        SELECT 1 
        FROM coupons 
        WHERE user_id = p_user_id 
    ...
    IF v_current_email_usage < v_monthly_email_limit AND v_active_campaign IS NOT NULL AND v_has_stock THEN
    ...
    ELSE
         v_result := jsonb_build_object(
            ...
            'reason', CASE 
                ...
                WHEN NOT v_has_stock THEN 'no_stock'
                ELSE 'unknown'
            END,
            'config', COALESCE(v_widget_config, '{"cooldown_minutes": 1440}'::jsonb)
        );
